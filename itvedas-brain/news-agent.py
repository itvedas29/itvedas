...
# regenerated by this script on every run, so a one-off edit to the
# committed HTML doesn't stick - it has to live here or the next run
# reverts it to a flat nav-links list, which is exactly what happened
# after the site-wide nav rollout.
...
<div class="hero">
  <div class="breadcrumb"><a href="/">Home</a> › <a href="/news">News</a> › {topic}</div>
  <div class="badges">
...
def build_news_index(articles, update_time):
    ...
    for a in articles:
        ...
        cards += f"""<a href="/news/{a['slug']}" class="nc" data-t="{a['topic']}">
...
<link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;600;700&family=Inter:wght@400;500&display=swap" rel="stylesheet">
<style>
:root{{--bg:#EEF3FC;--bg2:#FFFFFF;--bg3:#F7F9FD;--text:#182238;--muted:#6B7A94;--sub:#4A5568;--accent:#FF6B35;--border:rgba(24,34,56,0.12);}}
*,*::before,*::after{{box-sizing:border-box;margin:0;padding:0;}}
html{{scroll-behavior:smooth;}}
...
.fb:hover{{color:var(--text);border-color:rgba(24,34,56,0.22);}}
...
def build_security_news_page(articles, update_time):
    ...
    for a in security_articles:
        ...
        if a.get('topic') == 'CVE' and a.get('cve_id'):
            ...
            cards += f"""<a href="/news/{a['slug']}" class="nc cve-card" data-t="CVE">
              <div class="nc-body">
...
<link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;600;700&family=Inter:wght@400;500&display=swap" rel="stylesheet">
<style>
:root{{--bg:#EEF3FC;--bg2:#FFFFFF;--bg3:#F7F9FD;--text:#182238;--muted:#6B7A94;--sub:#4A5568;--accent:#10B981;--border:rgba(24,34,56,0.12);}}
*,*::before,*::after{{box-sizing:border-box;margin:0;padding:0;}}
html{{scroll-behavior:smooth;}}
body{{background:var(--bg);color:var(--text);font-family:'Inter',sans-serif;-webkit-font-smoothing:antialiased;}}
nav{{position:fixed;top:0;left:0;right:0;z-index:100;display:flex;align-items:center;justify-content:space-between;padding:0 2rem;height:64px;background:rgba(255,255,255,0.88);backdrop-filter:blur(20px);border-bottom:1px solid var(--border);}}
...
<footer>
  <div class="fl">IT<span>Vedas</span></div>
  <p>Original IT news and guides — explained simply, for everyone.</p>
  <div class="flinks"><a href="/">Home</a><a href="/news">All News</a><a href="/news/archive/">Full Archive</a><a href="/#chapters">Chapters</a><a href="mailto:info@itvedas.com">Contact</a><a href="/sitemap.xml">Sitemap</a></div>
  <p style="margin-top:1rem;">© {datetime.date.today().year} ITVedas · Original reporting</p>
</footer>
<script>
function f(t){{
...
def update_homepage_security(published):
    ...
    for a in recent:
        ...
        cards += (
            f'<a href="/news/{a["slug"]}" class="lsc">'
    ...
    section = (
        ...
        '<a href="/security-news" class="live-security-link">See all security news →</a>'
        '</div>'
...
def main():
    ...
    if new_urls:
        _indexnow_submit(new_urls + ["/news", "/security-news", "/"], log_fn=log)
