...
class TestDirectoryStructure(unittest.TestCase):
    ...
    def test_script_files_are_executable(self):
        ...
        scripts_dir = Path(str(REPO_ROOT) + "/scripts")
...
class TestAssetAvailability(unittest.TestCase):
    ...
    def test_logo_referenced_in_html(self):
        ...
        index = Path(str(REPO_ROOT) + "/index.html")
        if index.exists():
...
class TestConfigurationFiles(unittest.TestCase):
    ...
    def test_pytest_config_exists(self):
        ...
        pytest_ini = Path(str(REPO_ROOT) + "/pytest.ini")
        self.assertTrue(pytest_ini.exists())

    def test_deploy_config_exists(self):
        """Cloudflare Pages deploy config (wrangler.toml, _headers, _redirects) should exist"""
        self.assertTrue((REPO_ROOT / "wrangler.toml").exists())
        self.assertTrue((REPO_ROOT / "_headers").exists())
        self.assertTrue((REPO_ROOT / "_redirects").exists())

    def test_gitignore_exists(self):
        """.gitignore should exist"""
        gitignore = Path(str(REPO_ROOT) + "/.gitignore")
        self.assertTrue(gitignore.exists())

    def test_security_headers_in_netlify(self):
        """Security headers should be configured in netlify.toml"""
        netlify = Path(str(REPO_ROOT) + "/netlify.toml")
        if netlify.exists():
