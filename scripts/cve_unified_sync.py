...
NVD is canonical. CISA KEV is authoritative for known exploitation.
GitHub Global Security Advisories are optional supplemental references.
"""
import json, os, time
...
def main():
 ...
 start=0
 while True:
  params={'resultsPerPage':100,'startIndex':start,'lastModStartDate':checkpoint.strftime('%Y-%m-%dT%H:%M:%S.000Z'),'lastModEndDate':now().strftime('%Y-%m-%dT%H:%M:%S.000Z')};h={'apiKey':os.getenv('NVD_API_KEY')} if os.getenv('NVD_API_KEY') else HEADERS;data,_=get(NVD,params,h);items=data.get('vulnerabilities',[])
  for wrap in items:
