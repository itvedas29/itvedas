...
Marks thin or near-duplicate stories noindex and writes a report. The oldest
story in a duplicate cluster remains indexable; later rewrites are retained
for users but removed from the XML sitemap by generate-sitemap.py.
"""
...
def set_noindex(path):
    ...
    if re.search(r'<meta[^>]+name=["\']robots["\']',raw,re.I):
        raw=re.sub(r'(<meta[^>]+name=["\']robots["\'][^>]+content=["\'])index,follow(["\'])',r'\1noindex,follow\2',raw,count=1,flags=re.I)
    else: raw=raw.replace('<head>','<head>\n<meta name="robots" content="noindex,follow">',1)
    path.write_text(raw,encoding='utf-8'); return True

def main():
    pages=[]
    for p in sorted(ROOT.rglob('*.html')):
        raw=p.read_text(encoding='utf-8',errors='ignore'); title,canonical=metadata(raw); body=text(raw)
        pages.append({'path':p,'title':title,'canonical':canonical,'words':len(body.split()),'raw':raw})
    ...
    for i,p in enumerate(pages):
        if p['words']<MIN_WORDS:
            ...
            continue
        dup=False
        for older in pages[:i]:
            if older['words']<MIN_WORDS: continue
            if p['canonical'] and older['canonical'] and p['canonical']==older['canonical']: dup=True
            elif sim(p['title'],older['title'])>=0.78: dup=True
            if dup: break
        if dup:
